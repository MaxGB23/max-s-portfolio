...
- [x] **Decisión final del propietario (breakpoint 768)**: el régimen del card usa `landscape:lg:[@media(max-height:768px)]` (mb-24 y last:mb-0) deliberadamente — cubre el píxel exacto 768 que quedaba mal con 767; testeado en vivo 767/768/769 sin romper nada. El spacer `projects-all-projects` de página sigue en 767 (estado aceptado). Clase final del article: `portrait:lg:mb-24 landscape:lg:[@media(max-height:768px)]:mb-24 landscape:lg:[@media(max-height:768px)]:last:mb-0`. Contrato QA: `featured-card-card` landscape lg ≤768 → 96±10; `projects-all-projects` landscape lg ≤767 → `{ min: spacing }`.
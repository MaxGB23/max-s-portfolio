...
| featured-card-card | landscape lg AND height ≤768 | `96±10` (`landscape:lg:[@media(max-height:768px)]:mb-24 landscape:lg:[@media(max-height:768px)]:last:mb-0`) |
...
- [x] T6 … (hecho) — `landscape:lg:[@media(max-height:768px)]:mb-24` añadido al `article` de `featured-project-panel.tsx` justo tras `portrait:lg:mb-24` (espejo del régimen `projects-all-projects` de `lib/rhythm.ts`); landscape lg corto → 96px entre cards
...
- [x] **Decisión final del propietario (breakpoint 768)**: el régimen del card usa `landscape:lg:[@media(max-height:768px)]` (mb-24) deliberadamente — cubre el píxel exacto 768 que quedaba mal con 767; testeado en vivo 767/768/769 sin romper nada. El `last:mb-0` quedó en **767** (ajuste del propietario pre-merge): espejo exacto del spacer `projects-all-projects`; en el píxel 768 la gate GSAP es dueña y el margen de la última card es irrelevante. Clase final del article: `portrait:lg:mb-24 landscape:lg:[@media(max-height:768px)]:mb-24 landscape:lg:[@media(max-height:767px)]:last:mb-0`. Contrato QA: `featured-card-card` landscape lg ≤768 → 96±10; `projects-all-projects` landscape lg ≤767 → `{ min: spacing }`.